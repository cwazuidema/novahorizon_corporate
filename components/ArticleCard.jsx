...
export default function ArticleCard({ article, className = '' }) {
    return (
        <Link href={`/artikelen/${article.slug}`} className="block">
            <Card className={["flex flex-col cursor-pointer", className].filter(Boolean).join(' ')}>
                ...
                <div className="flex flex-1 flex-col p-5">
                    ...
                    <div className="mt-auto flex items-center justify-between">
                        ...
                        <span className="inline-flex items-center justify-center rounded-2xl px-4 py-2 text-sm font-medium text-primary transition hover:bg-secondary">Lees verder</span>
                    </div>
                </div>
            </Card>
        </Link>
